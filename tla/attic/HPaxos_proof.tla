(***************************************************************************)
(* Compiles with TLAPM version 1.4.5 (commit 33809)                        *)
(***************************************************************************)
...
-----------------------------------------------------------------------------
MaxBalSpec ==
    \A L \in Learner : \A A \in SafeAcceptor : \A B \in Ballot :
        maxBal[L, A] =< B => ~LeftBallot(L, A, B)
...
LEMMA MaxBalSpecInvariant == TypeOK /\ Next /\ MaxBalSpec => MaxBalSpec'
PROOF
<1> SUFFICES ASSUME TypeOK, Next, MaxBalSpec
PROVE MaxBalSpec' OBVIOUS
<1> USE DEF MaxBalSpec
<1>0. TypeOK' BY TypeOKInvariant
<1>1. CASE ProposerAction
  <2>0. SUFFICES ASSUME NEW lrn \in Learner, NEW bal \in Ballot, NEW val \in Value,
                        Phase1a(lrn, bal) \/ Phase1c(lrn, bal, val)
                 PROVE MaxBalSpec'
        BY <1>1, ValueNotEmpty DEF ProposerAction
  <2>1. CASE Phase1a(lrn, bal) BY Isa, <2>1 DEF Phase1a, Send, LeftBallot
  <2>2. CASE Phase1c(lrn, bal, val) BY Isa, <2>2 DEF Phase1c, Send, LeftBallot
  <2>3. QED BY <2>0, <2>1, <2>2
<1>2. CASE AcceptorSendAction
  <2>0. SUFFICES ASSUME NEW lrn \in Learner, NEW bal \in Ballot,
                        NEW acc \in SafeAcceptor, NEW val \in Value,
                        \/ Phase1b(lrn, bal, acc)
                        \/ Phase2av(lrn, bal, acc, val)
                        \/ Phase2b(lrn, bal, acc, val)
                 PROVE MaxBalSpec'
  ...
  <2>1. CASE Phase1b(lrn, bal, acc)
    <3> SUFFICES ASSUME NEW L \in Learner, NEW A \in SafeAcceptor, NEW B \in Ballot,
                        maxBal'[L, A] =< B,
                        NEW M \in msgs', M.type = "1b", M.lr = L, M.acc = A, B < M.bal
                 PROVE FALSE
        BY <2>1 DEF Phase1b, Send, LeftBallot
    <3>1. CASE M \in msgs
      <4>1. maxBal[L, A] =< maxBal'[L, A] BY MaxBalMonotone
      <4>1a. maxBal[L, A] \in Ballot BY SafeAcceptorIsAcceptor DEF TypeOK
      <4>1b. maxBal'[L, A] \in Ballot BY SafeAcceptorIsAcceptor, <1>0 DEF TypeOK
      <4>2. maxBal[L, A] =< B BY <4>1, <4>1a, <4>1b, BallotLeqTrans
      <4>3. QED BY <3>1, <4>2 DEF LeftBallot
    <3>2. CASE M \notin msgs
      <4>1. M = [ type |-> "1b",
                  lr |-> lrn,
                  acc |-> acc,
            ...
            BY <2>1, <3>2 DEF Phase1b, Send
      <4>2. L = lrn /\ A = acc BY <4>1
      <4>3. maxBal'[lrn, acc] = bal BY <2>1, <1>0, SafeAcceptorIsAcceptor DEF Phase1b, TypeOK
      <4>4. maxBal'[L, A] = bal BY <4>3, <4>2
      <4>5. bal =< B BY <4>4
      <4>6. B < bal BY <4>1
      <4>10. QED BY <4>5, <4>6, BallotLeNotLeq
    <3>3. QED BY <3>1, <3>2
  <2>2. CASE Phase2av(lrn, bal, acc, val) BY Isa, <2>2 DEF Phase2av, Send, LeftBallot
  <2>3. CASE Phase2b(lrn, bal, acc, val) BY Isa, <2>3 DEF Phase2b, Send, LeftBallot
  <2>4. QED BY <2>0, <2>1, <2>2, <2>3
<1>3. CASE AcceptorReceiveAction BY <1>3 DEF AcceptorReceiveAction, Recv, LeftBallot
<1>4. CASE AcceptorDisconnectAction BY <1>4 DEF AcceptorDisconnectAction, Disconnect, LeftBallot
<1>5. CASE LearnerAction BY <1>5 DEF LearnerAction, LearnerDecide, LearnerRecv, LeftBallot
<1>6. CASE FakeAcceptorAction
      BY <1>6, SafeAcceptorAssumption DEF FakeAcceptorAction, FakeSend, Send, LeftBallot
<1>7. QED BY <1>1, <1>2, <1>3, <1>4, <1>5, <1>6 DEF Next
...
<1>2av. ASSUME TypeOK, Next,
  ...
  <2>1. CASE ProposerAction
    <3>0. /\ m \in msgs
          ...
          /\ m.bal \in Ballot
          /\ m.val \in Value
          BY <1>2av, <2>1, <2>0e, <2>0b, MessageType
          DEF ProposerAction, Phase1a, Phase1c, MsgInv2av, Next, Send, TypeOK
    <3>1. UNCHANGED <<maxBal, votesSent, 2avSent, received, connected, receivedByLearner, decision>>
          BY <2>1 DEF Phase1a, Phase1c, ProposerAction
  ...
  <2>2. CASE AcceptorSendAction
    ...
    <3>1. CASE Phase1b(lrn, bal, acc)
      <4>0. /\ m \in msgs
            ...
            /\ m.bal \in Ballot
            /\ m.val \in Value
            BY <1>2av, <3>1, <2>0e, <2>0b, MessageType DEF Phase1b, Send, TypeOK
      <4>1. UNCHANGED <<votesSent, 2avSent, received, connected, receivedByLearner, decision>>
            BY <3>1 DEF Phase1b, Send
      <4>2. InitializedBallot(m.lr, m.bal)'
            BY <4>0, <1>2av, InitializedBallotInv DEF MsgInv2av
      <4>3. AnnouncedValue(m.lr, m.bal, m.val)'
            BY <4>0, <1>2av, AnnouncedValueInv DEF MsgInv2av
      <4>4. KnowsSafeAt(m.lr, m.acc, m.bal, m.val)'
            BY <4>0, <1>2av, KnowsSafeAtInv DEF MsgInv2av
    ...
    <3>2. CASE Phase2av(lrn, bal, acc, val)
      ...
      <4>1. CASE m \in msgs
        <5>0. /\ m \in msgs
              ...
              /\ m.bal \in Ballot
              /\ m.val \in Value
              BY <4>1, <1>2av, <3>1, <2>0e, <2>0b, MessageType DEF Phase1b, Send, TypeOK
        <5>1. InitializedBallot(m.lr, m.bal)'
              BY <5>0, <1>2av, InitializedBallotInv DEF MsgInv2av
        <5>2. AnnouncedValue(m.lr, m.bal, m.val)'
              BY <5>0, <1>2av, AnnouncedValueInv DEF MsgInv2av
        <5>3. KnowsSafeAt(m.lr, m.acc, m.bal, m.val)'
              BY <5>0, <1>2av, KnowsSafeAtInv DEF MsgInv2av
        <5>4. QED BY <5>0, <5>1, <5>2, <5>3, <1>2av, 2avSentMonotone DEF MsgInv2av
      <4>2. CASE m \notin msgs
        <5>1. m = [type |-> "2av", lr |-> lrn, acc |-> acc, bal |-> bal, val |-> val] BY <4>2 DEF Send
        <5>3. InitializedBallot(m.lr, m.bal)'
              BY <5>1, <1>2av, MsgsMonotone DEF InitializedBallot
        <5>4. AnnouncedValue(m.lr, m.bal, m.val)'
              BY <5>1, <1>2av, MsgsMonotone DEF AnnouncedValue
        <5>5. KnowsSafeAt(m.lr, m.acc, m.bal, m.val)'
          <6>1. KnowsSafeAt(m.lr, m.acc, m.bal, m.val) BY <5>1
          <6>10. QED BY <6>1 DEF KnowsSafeAt, KnowsSafeAt1, KnowsSafeAt2
        <5>6. ([lr |-> m.lr, bal |-> m.bal, val |-> m.val] \in 2avSent[m.acc])'
              BY <5>1, <2>0b DEF TypeOK
    ...
    <3>3. CASE Phase2b(lrn, bal, acc, val)
      <4>0. /\ m \in msgs
            ...
            /\ m.bal \in Ballot
            /\ m.val \in Value
            BY <1>2av, <3>3, <2>0b, MessageType DEF Phase2b, Send, TypeOK
      <4>1. UNCHANGED <<maxBal, 2avSent, received, connected, receivedByLearner, decision>>
...
LEMMA MaxBalSpecInit == Init => MaxBalSpec
PROOF BY DEF Init, MaxBalSpec, LeftBallot
...
PROOF BY TypeOKInit, MsgInvInit, MaxBalSpecInit,
...
<1>1. CASE Next BY <1>1,
        TypeOKInvariant, MsgInvInvariant, MaxBalSpecInvariant,
...
<1>2. CASE vars = vars' BY <1>2 DEF vars, FullSafetyInvariant, TypeOK, MsgInv,
          ...
          Safety,
          ChosenIn, HeterogeneousSpec, CannotDecide, LeftBallot, VotedFor, Proposed, MaxVote,
          InitializedBallot, AnnouncedValue, KnowsSafeAt, KnowsSafeAt1, KnowsSafeAt2
<1>3. QED BY <1>1, <1>2
